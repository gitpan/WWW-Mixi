...
なお、loginメソッド同様、要ログイン時のレスポンスコードは401です。
...
=item $mixi->enbale_cookies($cookie_file)
...
AsO (http://www.bx.sakura.ne.jp/~clan/rn/cgi-bin/index.cgi) for method to parse "send message".
makamaka (http://www.donzoko.net/) for some bugfixes.
