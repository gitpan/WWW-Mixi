...
=item $response = $mixi->request($request, $arg [, $size]) )
...
=item $item = $mixi->parse_calendar_term( [$response] );
...
=item $item = $mixi->parse_calendar_next( [$response] );
...
=item $item = $mixi->parse_calendar_previous( [$response] );
...
=item $item = $mixi->parse_list_community_next( [$response] );
...
=item $item = $mixi->parse_list_community_previous( [$response] );
...
=item $item = $mixi->parse_list_diary_capasity( [$response] );
...
=item $item = $mixi->parse_list_diary_next( [$response] );
...
=item $item = $mixi->parse_list_diary_previous( [$response] );
...
=item $item = $mixi->parse_list_friend_next( [$response] );
...
=item $item = $mixi->parse_list_friend_previous( [$response] );
...
=item @items = $mixi->parse_list_outbox( [$response] );
...
=item @items = $mixi->parse_new_diary( [$response] );
...
=item $item = $mixi->parse_new_diary_next( [$response] );
...
=item $item = $mixi->parse_new_diary_previous( [$response] );
...
=item $item = $mixi->parse_new_friend_diary_next( [$response] );
...
=item $item = $mixi->parse_new_friend_diary_previous( [$response] );
...
=item $item = $mixi->parse_show_friend_outline( [$response] );
...
=item $item = $mixi->parse_show_friend_profile( [$response] );
...
=item $item = $mixi->parse_view_diary( [$response] );
...
=item $item = $mixi->parse_view_message( [$response] );
...
=item $item = $mixi->parse_add_diary_confirm( [$response ]);
...
=item @items = $mixi->parse_delete_diary_preview( [$response ]);
...
=item $item = $mixi->parse_delete_diary_confirm( [$response ]);
...
=item @items = $mixi->parse_edit_diary_preview( [$response ]);
...
=item @items = $mixi->parse_edit_diary_image( [$response ]);
...
=item $item = $mixi->parse_edit_diary_confirm( [$response ]);
...
=item $item = $mixi->parse_send_message_preview( [$response ]);
...
=item $item = $mixi->parse_send_message_confirm( [$response ]);
...
=item $item = $mixi->get_calendar_term( [$url | $url => 'refresh' | 'refresh'] );
...
=item $item = $mixi->get_calendar_next( [$url | $url => 'refresh' | 'refresh'] );
...
=item $item = $mixi->get_calendar_previous( [$url | $url => 'refresh' | 'refresh'] );
...
=item $item = $mixi->get_list_community_next( [$url | $url => 'refresh' | 'refresh'] );
...
=item $item = $mixi->get_list_community_previous( [$url | $url => 'refresh' | 'refresh'] );
...
=item $item = $mixi->get_list_diary_capacity( [$url | $url => 'refresh' | 'refresh'] );
...
=item $item = $mixi->get_list_diary_next( [$url | $url => 'refresh' | 'refresh'] );
...
=item $item = $mixi->get_list_diary_previous( [$url | $url => 'refresh' | 'refresh'] );
...
=item @items = $mixi->get_list_outbox( [$url | $url => 'refresh' | 'refresh'] );
...
=item @items = $mixi->get_new_diary( [$url | $url => 'refresh' | 'refresh']  [,'keyword' => $keyword]);
...
=item $item = $mixi->get_new_diary_next( [$url | $url => 'refresh' | 'refresh'] [,'keyword' => $keyword]);
...
=item $item = $mixi->get_new_diary_previous( [$url | $url => 'refresh' | 'refresh']  [,'keyword' => $keyword]);
...
=item $item = $mixi->get_show_friend_outline( $url [ => 'refresh'] );
...
=item $item = $mixi->get_show_friend_profile( $url [ => 'refresh'] );
...
=item $item = $mixi->get_view_diary( $url [ => 'refresh'] );
...
=item @items = $mixi->get_add_diary_confirm('diary_title' => '日記タイトル', 'diary_body' => '日記本文' [, 'photo1' => '写真1パス'] [, 'photo2' => '写真2パス'] [, 'photo3' => '写真3パス']);

日記を投稿します。
返り値はparse_add_diary_confirmと同じです。

=item @items = $mixi->get_delete_diary_preview('id' => '日記ID');

日記削除の確認画面を取得し、解析します。
返り値はparse_delete_diary_previewと同じです。

=item @items = $mixi->get_delete_diary_confirm('id' => '日記ID');

日記を削除します。
返り値はparse_delete_diary_confirmと同じです。

=item @items = $mixi->get_edit_diary_preview($url|'id' => '日記ID');

日記を編集（再投稿）ページを取得し、解析します。
返り値はparse_edit_diary_previewと同じです。

=item @items = $mixi->get_edit_diary_image($url|'id' => '日記ID');;

日記を編集（再投稿）ページを取得し、解析します。
返り値はparse_edit_diary_imageと同じです。

=item @items = $mixi->get_edit_diary_confirm('id' => '日記ID', 'diary_title' => '日記タイトル', 'diary_body' => '日記本文' [, 'photo1' => '写真1パス'] [, 'photo2' => '写真2パス'] [, 'photo3' => '写真3パス']);

日記を編集（再投稿）します。
返り値はparse_edit_diary_confirmと同じです。

=item @items = $mixi->get_send_message_preview('id' => '受信者のID', 'subject' => 'メッセージのタイトル', 'body' => 'メッセージの本文');

メッセージ送信ページに引数のデータを送信し、プレビューページを取得して解析します。
返り値はparse_send_message_previewと同じです。

=item @items = $mixi->get_send_message_confirm('id' => '受信者のID', 'subject' => 'メッセージのタイトル', 'body' => 'メッセージの本文' [, 'post_key' => '送信キー']);
...
=item $url = $mixi->query_sorted_url($url)
...
=item $mixi->enable_cookies($cookie_file)
...
=item $str = $mixi->escape($str);

HTMLエスケープした文字列を返します。

=item $str = $mixi->unescape($str);
...
=item @items = $mixi->parse_standard_form( [$response] );
...
いくつかのケースでは、mixiがレスポンスコード"200"などを返す場合でも、実際のコンテンツの内容からWWW::Mixiがレスポンスコードを400に置き換えます。
mixiからのレスポンスコードが当初から400である可能性もあるため、原因はレスポンスメッセージを確認してください。
...
すべてのリクエストについて、取得したコンテンツにログインフォームが表示されている場合は、レスポンスコードは401になります。
...
エラーメッセージがなく、ログインフォームだけが表示されています。
...
Some bug fixes submitted by Topia (http://clovery.jp/), shino (http://www.freedomcat.com/), makamaka (http://www.donzoko.net/), あっしゅ.
