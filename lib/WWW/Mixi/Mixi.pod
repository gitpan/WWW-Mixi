...
parse_show_calendarメソッドの別名です。

mixiのカレンダ表示URLの変更に伴い、メソッド名が変更されました。
現時点では互換性の観点からこのメソッド名でも利用できるようになっていますが、将来の版ではこのメソッドは使用できなくなります。

=head3 $item = $mixi->parse_calendar_term( [$response] );

parse_show_calendar_termメソッドの別名です。
...
parse_show_calendar_nextメソッドの別名です。
...
parse_show_calendar_previousメソッドの別名です。
...
=head3 $item = $mixi->parse_new_bbs_next( [$response] ); New!
...
=head3 $item = $mixi->parse_new_bbs_previous( [$response] ); New!
...
parse_search_diaryメソッドの別名です。

mixiの新着日記検索URLの変更に伴い、メソッド名が変更されました。
現時点では互換性の観点からこのメソッド名でも利用できるようになっていますが、将来の版ではこのメソッドは使用できなくなります。

=head3 $item = $mixi->parse_new_diary_next( [$response] );

parse_search_diary_nextメソッドの別名です。
...
parse_search_diary_previousメソッドの別名です。
...
get_show_calendar_termメソッドの別名です。
...
get_show_calendar_nextメソッドの別名です。
...
get_show_calendar_previousメソッドの別名です。
...
get_search_diary_nextメソッドの別名です。
...
get_search_diary_previousメソッドの別名です。
...
=head3 $item = $mixi->get_view_bbs( $url [ => 'refresh'] ); New!
...
=head3 $response = $mixi->post_add_diary('submit' => $action, 'diary_title' => $title, 'diary_body' => $body, [, 'photo1' => $path_to_photo1, 'photo2' => $path_to_photo2, 'photo3' => $path_to_photo3] [, 'orig_size' => 1]] [, 'packed' => $hash_value]);
...
=head3 $response = $mixi->post_edit_diary('submit' => $action, 'diary_id' => $id, 'diary_title' => $title, 'diary_body' => $body, 'photo1' => $path_to_photo1, 'photo2' => $path_to_photo2, 'photo3' => $path_to_photo3);
...
=head3 $response = $mixi->post_delete_diary('submit' => $action, 'diary_id' => $id);
